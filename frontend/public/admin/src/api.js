...
class AdminAPI {
  constructor() {
    this.baseURL = window.AdminConfig?.API_BASE_URL || 'https://ecoa-nine.vercel.app';
    this.token = localStorage.getItem(window.AdminConfig?.AUTH?.TOKEN_KEY || 'admin_token');
  }
  ...
  async request(endpoint, options = {}) {
    ...
    try {
      ...
      if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
        throw new Error('Error de conexión: El servidor no permite peticiones desde este origen. Verifica la configuración CORS del backend.');
      }
  ...
  async getStats() {
    ...
    return {
      ...
      plants: {
        ...
        healthy: plants.data.filter(p => p.health_status === 'healthy').length,
        needsCare: plants.data.filter(p => p.health_status === 'needs_care').length
      },
      donations: {
        total: donations.count,
        active: activeDonations.length,
        completed: completedDonations.length,
        totalAmount: donations.data.reduce((sum, d) => sum + (d.amount || 0), 0)
      }
    };
  }
