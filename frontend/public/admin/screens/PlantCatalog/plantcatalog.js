...
async function loadDevices() {
  try {
    ...
    // Función helper para crear el label del dispositivo
    const createDeviceLabel = (device) => {
      ...
      return raspberryBadge + (parts.length > 0 ? parts.join(' | ') : device.id) + lastConnText;
    };
    ...
    if (overlayDeviceSelect) {
      ...
      sortedDevices.forEach(device => {
        ...
        option.textContent = createDeviceLabel(device);
