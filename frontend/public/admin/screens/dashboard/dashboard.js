...
async function loadStats() {
  try {
    ...
    // Total Adoptions (plantas adoptadas)
    updateStatCard("totalUsers", stats.plants.adopted || 0);
    updateStatSubtext("newUsers", `${adoptedThisMonth} nuevos este mes`);
    ...
    // Health Score - calcular basado en plantas saludables vs total
    const healthScore = (() => {
      const total = stats.plants.total || 0;
      if (!total) return "0%";
      
      // Calcular plantas saludables (healthy + excellent mapeado a healthy)
      // Como health_status viene de plant_status, necesitamos obtenerlo de otra forma
      // Por ahora usamos el filtro que ya existe en getStats
