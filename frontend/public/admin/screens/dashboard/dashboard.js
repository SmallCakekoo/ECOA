...
async function loadRecentPlants() {
  try {
    ...
    if (container) {
      ...
      const plantRows = await Promise.all(plants.map(async (plant) => {
        ...
        for (const searchTerm of searchTerms) {
          try {
            const perenualResponse = await fetch(`http://localhost:3001/api/integrations/perenual/search?q=${encodeURIComponent(searchTerm)}&limit=1`);
            ...
            if (perenualData.success && perenualData.search_results?.plants?.length > 0) {
              ...
              console.log(`🔍 Datos de imagen para ${plant.name}:`, perenualPlant.default_image);
              
              // Verificar que la imagen no sea un placeholder genérico
              const imageUrl = perenualPlant.default_image?.thumbnail || 
                              perenualPlant.default_image?.small_url || 
                              perenualPlant.default_image?.medium_url || 
                              perenualPlant.default_image?.regular_url;
              
              if (imageUrl && !imageUrl.includes('upgrade_access.jpg') && !imageUrl.includes('placeholder')) {
                plantImage = imageUrl;
                console.log(`✅ Imagen real encontrada para ${plant.name} con término: "${searchTerm}" - URL: ${plantImage}`);
                break;
              } else {
                console.log(`❌ Imagen placeholder detectada para ${plant.name} con término: "${searchTerm}" - URL: ${imageUrl}`);
              }
            }
          } catch (error) {
            console.log(`Error buscando "${searchTerm}" para ${plant.name}:`, error);
          }
        }
        
        if (!plantImage) {
          console.log(`❌ No se encontró imagen real para ${plant.name} (${plant.species})`);
        } else {
...
async function createPlant() {
  ...
  const imageUrl = photoPreview && photoPreview.src && photoPreview.src !== '' ? photoPreview.src : null;
